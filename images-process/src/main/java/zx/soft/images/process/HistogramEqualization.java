...
public class HistogramEqualization extends Configured implements Tool {
	...
	public static class HE_Map extends Mapper<ImageHead, RawImage, IntWritable, RawImage> {
		...
		public void map(ImageHead key, RawImage value, Context context) throws IOException, InterruptedException {
			System.out.println("A map started.");
			...
			for (int i = 1; i < 256; i++) {
				sum[i] += sum[i - 1];
				cdf[i] = (sum[i] - 1) * 255 / (totalPixels - 1);
			}
			for (int i = 0; i < totalPixels; i++) {
				newY[i] = cdf[Y[i]];
			}
			...
			RawImage image = new RawImage(imageWidth, imageHeight, newY, newY, newY, value.getY(), value.getCb(),
					value.getCr());
	...
	//	public static class HE_Reduce extends Reducer<IntWritable, RawImage, IntWritable, IntWritable> {
	//
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(conf, "HistogramEqualization");
		...
		job.setJarByClass(HistogramEqualization.class);
		job.setMapperClass(HE_Map.class);
		//		job.setReducerClass(HE_Reduce.class);
	...
	public static void main(String[] args) throws Exception {
		int exitCode = ToolRunner.run(new HistogramEqualization(), args);
